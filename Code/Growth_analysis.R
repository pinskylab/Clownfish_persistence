...
marked_fish <- marked_fish %>%  # 3053 individual fish on 10/15/19, 3812 on 6/12/20
...
# Pull out fish captured more than once - 503 obs (old: 464 obs), still 503 on 6/12/20
...
# Join pairs together into one data frame - 697 pairs (earlier was 772 pairs? - maybe PIT typos were fixed?) (was 545 pairs before addition of 2016-2018 genetic data), 763 pairs on 6/12/20
...
# Filter out just those recaptured about a year later - 244, 265 on 6/12/20
...
model_1_k <- as.numeric(-log(model_1$coefficients[2]))  # (0.848 on 6/12/20) 0.919 if use all fish captured about a year later (even multiple pairs per fish) --  0.881 one time, 0.938 another, pretty similar to fishmethods lowest AIC estimate of 0.867
model_1_Linf <- as.numeric(model_1$coefficients[1]/(1 - model_1$coefficients[2]))  # (10.763 on 6/12/20) 10.61 if use all fish captured about a year later (even multiple pairs per fish) --  10.69 one time, 10.58 another, pretty similar to fishmethods lowest AIC estimate of 10.64
