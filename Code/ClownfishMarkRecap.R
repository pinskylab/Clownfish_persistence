...
findDist <- function(sitevec, latvec, lonvec, capyearvec, gpxdf, divedf, sampleyear){
  ...
  for(i in 1:nsteps) {
    ...
    if(sampleyear >= testcapyear) {
      if(!is.na(testlat)) {
        #filter out dives at the right site in the right year # NOW THAT HAVE GENETIC RECAPS TOO, HAVE SOME FISH FROM SITES THAT WEREN'T RESAMPLED IN LATER YEARS - NEED TO FIGURE OUT HOW TO DEAL WITH THAT!!
...
# # Create a data frame to store anem_id where caught each year and lat/lon in each year
# encounters_all <- encounters_all %>% 
#   mutate(anem_2012 = rep(NA, length(fish_id)),
#          anem_2013 = rep(NA, length(fish_id)),
#          anem_2014 = rep(NA, length(fish_id)),
#          anem_2015 = rep(NA, length(fish_id)),
#          anem_2016 = rep(NA, length(fish_id)),
#          anem_2017 = rep(NA, length(fish_id)),
#          anem_2018 = rep(NA, length(fish_id)),
#          lat2012 = rep(NA, length(fish_id)), lon2012 = rep(NA, length(fish_id)),
#          lat2013 = rep(NA, length(fish_id)), lon2013 = rep(NA, length(fish_id)),
#          lat2014 = rep(NA, length(fish_id)), lon2014 = rep(NA, length(fish_id)),
#          lat2015 = rep(NA, length(fish_id)), lon2015 = rep(NA, length(fish_id)),
#          lat2012 = rep(NA, length(fish_id)), lon2012 = rep(NA, length(fish_id)),
#          lat2012 = rep(NA, length(fish_id)), lon2012 = rep(NA, length(fish_id)),
#          lat2012 = rep(NA, length(fish_id)), lon2012 = rep(NA, length(fish_id)),
#         capture_anem = rep(NA, length(fish_id))) # anem where fish was first captured
...
encounters_all$capture_anem_lat <- addLatLons(encounters_all, anem.Processed.TableIDs, "lat")
encounters_all$capture_anem_lon <- addLatLons(encounters_all, anem.Processed.TableIDs, "lon")
...
encounters_all$dist_2012 <- findDist(encounters_all$site, encounters_all$capture_anem_lat, encounters_all$capture_anem_lon, encounters_all$first_capture_year,gps.Info, dive.Info, 2012)
encounters_all$dist_2013 <- findDist(encounters_all$site, encounters_all$capture_anem_lat, encounters_all$capture_anem_lon, encounters_all$first_capture_year,gps.Info, dive.Info, 2013)
encounters_all$dist_2014 <- findDist(encounters_all$site, encounters_all$capture_anem_lat, encounters_all$capture_anem_lon, encounters_all$first_capture_year,gps.Info, dive.Info, 2014)
encounters_all$dist_2015 <- findDist(encounters_all$site, encounters_all$capture_anem_lat, encounters_all$capture_anem_lon, encounters_all$first_capture_year,gps.Info, dive.Info, 2015)
encounters_all$dist_2016 <- findDist(encounters_all$site, encounters_all$capture_anem_lat, encounters_all$capture_anem_lon, encounters_all$first_capture_year,gps.Info, dive.Info, 2016)
encounters_all$dist_2017 <- findDist(encounters_all$site, encounters_all$capture_anem_lat, encounters_all$capture_anem_lon, encounters_all$first_capture_year,gps.Info, dive.Info, 2017)
encounters_all$dist_2018 <- findDist(encounters_all$site, encounters_all$capture_anem_lat, encounters_all$capture_anem_lon, encounters_all$first_capture_year,gps.Info, dive.Info, 2018)
