...
# Vector of sites in alphabetical order when no spaces
...
# Error in parse for this one - not sure why.... think it might have been the spaces in the site names? But why wouldn't that have been a problem in other models too?
...
### Try surv by site for mean size (overall, not by site)
...
surv_by_site_mean_size <- data.frame(site = no_space_sites_alpha[1], size = mean_size_MARK) %>%
  mutate(estimate = results_df_1$estimate[1] + results_df_1$estimate[20]*size,
         lcl = results_df_1$lcl[1] + results_df_1$lcl[20]*size,
         ucl = results_df_1$ucl[1] + results_df_1$ucl[20]*size,
...
for(i in 2:length(no_space_sites_alpha)) {
  df_to_add <- data.frame(site = no_space_sites_alpha[i], size = mean_size_MARK) %>%
    mutate(estimate = results_df_1$estimate[1] + results_df_1$estimate[i] + results_df_1$estimate[20]*size,
           lcl = results_df_1$lcl[1] + results_df_1$lcl[i] + results_df_1$lcl[20]*size,
           ucl = results_df_1$ucl[1] + results_df_1$ucl[i] + results_df_1$ucl[20]*size,
           estimate_prob = logit_recip(estimate),
           lcl_prob = logit_recip(lcl),
           ucl_prob = logit_recip(ucl))
...
### Try surv by site for mean size by each site
...
### Now find relationship with size, not including site
# start with Cabatoan
...
recap_size_effect = 22

recap_by_size <- data.frame(size = size_values) %>%
  mutate(estimate = results_df_1$estimate[recap_intercept_index] + results_df_1$estimate[recap_size_effect]*size,
         lcl = results_df_1$lcl[recap_intercept_index] + results_df_1$lcl[recap_size_effect]*size,
         ucl = results_df_1$ucl[recap_intercept_index] + results_df_1$ucl[recap_size_effect]*size,
         estimate_prob = logit_recip(estimate),
         lcl_prob = logit_recip(lcl),
         ucl_prob = logit_recip(ucl))

### Find distance effect on recap
recap_dist_effect = 23

recap_by_dist <- data.frame(dist = dist_values) %>%
  mutate(estimate = results_df_1$estimate[recap_intercept_index] + results_df_1$estimate[recap_dist_effect]*dist,
         lcl = results_df_1$lcl[recap_intercept_index] + results_df_1$lcl[recap_dist_effect]*dist,
         ucl = results_df_1$ucl[recap_intercept_index] + results_df_1$ucl[recap_dist_effect]*dist,
         estimate_prob = logit_recip(estimate),
         lcl_prob = logit_recip(lcl),
         ucl_prob = logit_recip(ucl))

# Put best-fit model data frames together into a list to save
...
eall_mean0size_meanOdist.Phi.site.plus.size.p.size.plus.dist = mark(eall_meanYsize_meanYdist.processed_site, eall_meanYsize_meanYdist.ddl_site, model.parameters=list(Phi=Phi.site.plus.size, p=p.size.plus.dist), prefix = "eall_meanYdist_meanYsize.Phi.site.plus.size.p.size.plus.dist")  # site- and size-dependent survival, size-and-distance-dependent recapture
...
#################### Plots: ####################
##### Phi by site+size, recap by size+dist (eall_meanYsize_meanYdist.Phi.site.plus.size.p.size.plus.dist) (best-fit)
