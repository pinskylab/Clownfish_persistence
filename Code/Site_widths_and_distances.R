...
site_width_info <- site_edge_anems

# Pull out info on those anems so can use anem_table_ids in function to get lat/lon
site_width_anems <- anems_Processed %>%
  filter(anem_id %in% site_width_info$anem_id) %>%
  distinct(anem_id, .keep_all = TRUE)

# Match those anem_table_ids to the anem_ids in site_width_info
site_width_info <- left_join(site_width_info, site_width_anems %>% select(anem_table_id, anem_id), by = "anem_id")

# Find lat/lon for each of those anems
site_width_info <- site_width_info %>%
  mutate(lat = NA, lon = NA)

for(i in 1:length(site_width_info$anem_table_id)) {
  if (is.na(site_width_info$anem_table_id[i])) {  # some sites don't have a mid-anem and anemid_latlong function doesn't work if you put NA for anem_table_id
    site_width_info$lat[i] = NA
    site_width_info$lon[i] = NA
  } else {
    out_anemLL <- anemid_latlong(site_width_info$anem_table_id[i], anems_Processed, gps_Info)  
    site_width_info$lat[i] = out_anemLL$lat
    site_width_info$lon[i] = out_anemLL$lon
  }
}
...
#################### Saving output: ####################
save(site_width_info, file=here::here("Data/Script_outputs", "site_width_info.RData"))
save(site_dist_info, file=here::here("Data/Script_outputs", "site_dist_info.RData"))
