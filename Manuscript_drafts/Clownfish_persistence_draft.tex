...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% but in addition to tracking whether reproduction and survival of offp, such as a metapopulaton metapopulation persistence, however, we also care about connectivity - how offspring get distributed in space - in addition to the demographic processes. Connectivity is often described through dispersal kernels, probability density functions that give the relative number of settlers each distance from the origin patch \citep[e.g.][]{bode2018estimating}, which can then be discretized to form connectivity matrices that give the probability of dispersing from one patch to another (CITATIONS). Considering both demographic rates at the patches and the connectivity among them, a metapopulation can then persist in two ways: 1) at least one patch is able to achieve replacement through retention of osffspring produced locally (termed self-persistence) and 2) no patch retains enough local production to be able to persist in isolation but at least one patch receives enough offspring through connectivity loops with other patches to persist (termed network persistence) \citep{hastings_persistence_2006, burgess2014beyond}.


% In asssesing persistence in a spatially-structured metapopulation, we must consider both whether rindividuals replace themselvesFor any population to persist, individuals must on average replace themselves over their lifetime, and in a spatially-structure metapopulation, we must also consider how offspring get distributed across space. For consider both whether individuals produce engoughTo understand persistence of a metapopulation, we need to know both ho the demographic rates of the population - the production and survival of offspring - 
% ,

% For any population to persist, individuals must on average replace themselves during their lifetimes. We use various criteria for different types of populations, such as the average number of offspring contributed by each individual during its life (called $R_0$ when the population is age-structured and density-independent) or the growth rate of the population (such as the dominant eigenvalue $\lambda$ of an age-structured Leslie matrix) \citep{caswell_matrix_2001, burgess2014beyond}. Demographic persistence metrics take into account how likely individuals are to survive to the next age or stage, the expected fecundity at each stage, and the survival of any offspring produced to recruitment. CITATIONS? DOES THIS PARAGRAPH MAKE SENSE?

% For metapopulation persistence, however, we also care about connectivity - how offspring get distributed in space - in addition to the demographic processes. Connectivity is often described through dispersal kernels, probability density functions that give the relative number of settlers each distance from the origin patch \citep[e.g.][]{bode2018estimating}, which can then be discretized to form connectivity matrices that give the probability of dispersing from one patch to another (CITATIONS). Considering both demographic rates at the patches and the connectivity among them, a metapopulation can then persist in two ways: 1) at least one patch is able to achieve replacement through retention of osffspring produced locally (termed self-persistence) and 2) no patch retains enough local production to be able to persist in isolation but at least one patch receives enough offspring through connectivity loops with other patches to persist (termed network persistence) \citep{hastings_persistence_2006, burgess2014beyond}.

% % Mabye I should combine the demographic persistence paragraph with the connectivity part - topic sentence would be what? Need to understand both xx side of persistence and connectivity to understand metapops? Then a new paragraph about the persistence critera (self-persistence and network persistence) and how theory has helped us understand the nuances of those (give some summary, details here?). Then a new paragraph about how it's hard to actually get a sense of this empirically but we're getting there, both with better estiamtes of connectivity and now starting to get examples with combos of connectivity and demographics? Then a paragraph about what sets our case apart (fully sampled all habitat within a range that estiamted to be within this species' dispersal distance, sampling over time to get a more average sense of persistence, not just a couple of time points)

% % Need to come up with a better phrase for the demographic side of persistence
% Many models and simulations have helped us understand the nuances of persistence criteria and  


%  For metapopulations, however, we care about both the demographic processes - whether individuals in the patches produce enough surviving offspring to be able to replace themselves - and connectivity - how those offspring get distributed in space (CITATIONS). A metapopulation can persist in two ways: 1) at least one patch can achieve replacement through retention of offspring produced locally (termed self-persistence) and 

% Two ways a metapopulation can persist
% self-persistence, network persistence


% Or could have 1 paragraph about demographic persistence criteria
% Then one about connectivity and those kinds of metrics
% Then one about metapopulation persistence, where both demographics and connectivity are at play
% Then one about how this is hard to validate/test empirically but there has been progress, especially in connectivity. A long while of just focusing on connectivity, forgetting the demographics. But still gaps b/c we know these kinds of rates/parameters can vary over time and space, one snapshot isn't likely to tell us much about persistence in the long-run.

% Then one about how we are furthering this work with clownfish.
% -estimate inputs to our persistence metrics directly from our data
% -annual sampling across 8 year time period - can assess persistence with longer-term estimates of metrics, not just snapshot estimates
% -sampling a continuous section of a metapopulation that spans the estimated mean dispersal distance for this species, easily distinguished reef patches separated by non-habitat, sampled all habitat within this area
% -means we can compare our persistence metrics to population abundances through time, can assess whether this population is persistent and if so, how important connectivity between our sampled patches and outside patches is to persistence


% There are two wasyA patch could persist by producing sufficient offspring that disperse back to the patch to achieve replacement 

% the demographic piece - and Different types of populationWe have mathematical representations of this replacement for many types of populations, such as $R_0$ (CITATIONS).  on average   To persist, individuals in a population need to replace
% , where much Assessing connectivity and persistence for metapopulations in marine environments has  has long been a challenge becuase  among patches in metapopulations and determining how if it contributes to persistence of the metapopulation as a whole has long been a challenge in marine environments.

% Will need to address density-dependence and that these metrics are for low-densities when density-dependence is thought not to apply somewhere - probably both here to be clear about the metrics and in the methods somewhere to explain that we're basically ignoring the DD for now, even thought it is likely important in our system. Could also discuss in the results - particularly, egg-recruit survival would probably be higher in low-density situations (how much higher do we need it to be to see persistence? Is that an amount that could be explained by DD?)

% Brief summaries of papers and how they might fit in here:
% \begin{itemize}
% \item \cite{almany2017larval}: different species (but still tropical reef fish, including another clownfish species -  \textit{Amphiprion percula}), example of much better kernel fitting and estimates of larval connectivity using genetic parentage;
% \item \cite{salles2016genetic}: local habitat differences (like species of anemone) might lead to different demographic rates and production of recruits for the individuals in those habitats (here \textit{Amphiprion percula} on two anemone species in Kimbe Bay)
% \item \cite{lorenzen2018density}: in fisheries ecology, stock assessment, define recruit to be when a fish has grown/survived past the stage when density-dependence and environmental variability are really important for mortality, and has reached a stage when mortality is more stable and density-independent. Can be hard to figure out when that is, though, models can be mis-specified if get the timing/age/size of recruitment wrong and call things recruits too early.
% \end{itemize}
% %%%%%%%%%%%%%%%%%%%%% Outline below %%%%%%%%%%%%%%%%%%%%%%

% Understanding connectivity with metapopulations are connected and how the
...
We then multiply each size-distribution vector $v_z$ in the matrix by the size-dependent fecundity function described above (eqn.\ \ref{EQN_Fec}) to get the total number of eggs produced at each time step. To get the total number of eggs one individual is likely to produce in its lifetime, we then sum across all time steps in the individual's potential life.  % Should I try to show this in equation form? Not quite sure how that would look yet...
...
%\paragraph*{Exploring persistence requirements} - should this be its own paragraph in methods?
...
% Still need to discuss egg-recruit survival, size of transition to breeding female?, 
% List the parameter estimates here, cite relevant figures.
...
Based on our estimates of LRP, SP, and NP, we do not expect that our set of sites is able to persist in isolation as a closed system. To explore what would be required for persistence, we consider a hypothetical scenario in which we consider the system closed and say that all of the recruits arriving at our sites came from adults at our sites. In this case, we find a value of $\text{LRP} = 1.21$, above the value of 1 necessary for replacement (Fig.\ \ref{FIG_AllOffspringWhatIf}a). When we add in the connectivity, we see a higher value of network persistence in our best estimate ($\text{NP} = 0.20$) but still not high enough to indicate network persistence (Fig.\ \ref{FIG_AllOffspringWhatIf}b). We see more of the distribution of estimates above 1, however, suggesting that network persistence is within our range of uncertainty in this case, though not likely. With our site configuration and dispersal kernel estimate, we would need a value of LRP of XX (an egg-recruit survival of XX with our estimated value of LEP or a value of LEP of XX with our estimated value of egg-recruit survival).

%%%%% ADD TEXT, FIX CAPTION, LOOK AT DISCUSSION, THEN READ AND SEND OUT!
