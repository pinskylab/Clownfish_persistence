...
[\textit{Add in brief overview of genetic methods, with citations to papers with relevant methods and to Katrina's connectivity paper}]
...
Estimated abundance of females at each site over time is relatively constant [\textit{add some sort of actual analysis here}] (Fig.\ \ref{FIG_FthroughTime}, suggesting that our sample populations are persistent.
...
Using tagged and genotyped mark-recaptures, we estimate growth parameters XX (Fig.\ \ref{FIG_ParameterInputs}b) and survival parameters XX (Fig.\ \ref{FIG_ParameterInputs}c). [Put rest of actual parameter estimates here].
% List the parameter estimates here, cite relevant figures.

[\textit{Not sure where to put this, or if it should exist, but seems like it might be helpful - not fully filled out yet:}]
\begin{centering}
...
\begin{figure}[H] % demographic parameters: survival curve, growth curve, dispersal kernel, transition size to female 
	...
	\caption{Best estimates (solid black line) and range (gray) for dispersal, growth, survival, and size at female transition parameters. \label{FIG_ParameterInputs}}
\end{figure}
