...
\begin{figure}[H] % demographic parameters: survival curve, growth curve, dispersal kernel, transition size to female 
	...
	\caption{Estimates (solid black line) and uncertainty (grey) for a) dispersal (eqn.\ \ref{EQN_integratingDK}), b) growth (eqn.\ \ref{EQN_VBL}), including a dashed 1:1 line, c) post-recruit annual survival (eqn.\ \ref{APP_EQN_Survival}) at the example patch Elementary School, and d) raw data of fish size at female transition ($L_f$ in eqn.\ \ref{EQN_Fec}). \label{FIG_ParameterInputs}}
\end{figure}
...
\begin{figure}[H] % Schematic
	...
	\caption{The data collected for fish at each life stage and how they match to the equations and metrics estimated. We considered recruits to be offspring in their first year of settlement, represented by the 3.5--6.0 cm size range (SI \ref{APP_SEC_METHODS_Recruit_def}). \label{FIG_Schematic}} 
\end{figure}
...
\begin{figure}[!htbp] % site+size survival 
	...
	\caption{Annual post-recruit survival ($\phi$) by fish size at each patch, detailed in SI \ref{APP_SEC_METHODS_Growth_and_survival}, \ref{APP_SEC_Uncertainty}, and \ref{APP_SEC_RESULTS_Survival}. \label{APP_FIG_SurvBySizeAndSite}}
\end{figure}
...
\begin{figure}[!htbp] % distance and fish size effects on recapture probability from MARK
	...
	\caption{Effects of a) minimum distance between divers and the anemone where the fish was first caught and b) fish size on recapture probability $p_r$, estimated along with survival in a mark-recapture analysis (SI \ref{APP_SEC_METHODS_Growth_and_survival}, \ref{APP_SEC_RESULTS_Survival}). \label{APP_FIG_RecapSizeDistEffect}}
\end{figure}
